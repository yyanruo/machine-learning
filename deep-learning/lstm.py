...
class TanhActivator(object):
    def forward(self, w_input):
        return 2.0 / (1.0 + np.exp(-2 * w_input)) - 1.0
    def backward(self, output):
...
class LstmLayer(object):
    ...
    def calc_delta_k(self, k):
        ...
        delta_f = (delta_k * og * (
            1 - tanh_c * tanh_c) * c_prev * self.gate_activator.backward(fg))
        delta_i = (delta_k * og * (
            1 - tanh_c * tanh_c) * ct * self.gate_activator.backward(ig))
        delta_ct = (delta_k * og * (
            1 - tanh_c * tanh_c) * ig * self.output_activator.backward(ct))
        delta_h_prev = (np.dot(delta_o.transpose(), self.Woh) +
                        np.dot(delta_i.transpose(), self.Wih) +
                        np.dot(delta_f.transpose(), self.Wfh) + np.dot(
                            delta_ct.transpose(), self.Wch)).transpose()
